...
test('verify the days all select ',async({page}) => {
     await page.goto("https://testautomationpractice.blogspot.com/");
     const days:string[]=['Sunday','Monday','Wednesday','tuesday','thursday'];
...
//This script added on the 30/05/2026

test('Verify any one day select case', async ({page}) => {
        await page.goto("https://testautomationpractice.blogspot.com/");
         const days:string[]=['Sunday','Monday','Wednesday','tuesday','thursday'];

         const weekval:string="Monday"
         for(const lable of days){
            if(weekval==lable){
               const selectday= page.getByLabel(lable);
               selectday.check();
               await expect(selectday).toBeChecked();
            }
         }
  await page.waitForTimeout(4000);
});
